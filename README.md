...
| `SF_APP_IMPORT_MAX_LINES`                  | positive integer  |          | `1000`                                                    | `5000`                                                      | Maximum parsed data rows in one CSV import. The header and ignored blank rows do not count; this is independent of the upload byte-size limit.            |
...
`SF_APP_REGISTRATION_ENABLED` accepts `true` or `false`, and `SF_APP_IMPORT_MAX_LINES` must be an integer greater than zero. Invalid values prevent the backend from starting. Both settings are read at startup, so restart every application replica after changing them. A CSV with exactly `SF_APP_IMPORT_MAX_LINES` parsed data rows is accepted; the next row exceeds the limit.
