...
[![Coverage](https://img.shields.io/endpoint?url=https%3A%2F%2Fraw.githubusercontent.com%2Fynixt%2Fshared-finances%2Fmain%2F.github%2Fbadges%2Fjacoco-coverage.json)](https://raw.githubusercontent.com/ynixt/shared-finances/main/.github/badges/jacoco-coverage.json)
...
You can self-host this project using docker compose. First pick your version
at [GitHub Packages](https://github.com/ynixt/shared-finances/pkgs/container/shared-finances).
...
| `SF_APP_REGISTRATION_ENABLED`              | boolean                           |          | `true`                                                    | `false`                                                     | Enables new user registrations. When disabled, registration is blocked even for users arriving through an invitation; existing-user login remains available.                                                            |
| `SF_APP_LEGAL_DOCUMENTS_ENABLED`           | boolean                           |          | `false`                                                   | `true`                                                      | Presents the shipped terms of use and privacy policy together and records their versions at registration. The shipped text describes the maintainer's public instance; an operator enabling this for another instance must review and replace the content with documents for that operation. |
| `SF_APP_ENABLE_LIMITS`                     | boolean                           |          | `false`                                                   | `true`                                                      | Enables the plan model. When false, limits and quotas are not enforced or presented, limits pages and plan badges are hidden, and dormant-account deletion is disabled.                                                |
| `SF_APP_DEFAULT_ROLE`                      | `user`, `pro`, or `administrator` |          | `pro`                                                     | `user`                                                      | Plan role assigned only to accounts created after this value is set. Existing accounts keep their stored role.                                                                                                          |
...
| `SF_APP_LEGAL_TERMS_VERSION`               | date (yyyy-mm-dd)                 |          | `2026-08-10`                                              |                                                             | Date of terms of use                                                                                                                                                                                                    |
| `SF_APP_LEGAL_PRIVACY_VERSION`             | date (yyyy-mm-dd)                 |          | `2026-08-10`                                              |                                                             | Date of terms of privacy                                                                                                                                                                                                |
...
| `SF_APP_INACTIVE_ACCOUNT_DELETION_ENABLED` | boolean                           |          | `false`                                                   |                                                             | Setting it to `true` enables warning e-mails and **permanently deletes inactive accounts and their financial records** after the retention period configured for their plan. Owned groups are deleted with the account. |
| `SF_APP_INACTIVE_ACCOUNT_DELETION_CRON`    | string (cron)                     |          | `0 0 2 * * *`                                             |                                                             | Cron used to delete inactive accounts. Only if `SF_APP_INACTIVE_ACCOUNT_DELETION_ENABLED` is true.                                                                                                                      |

`SF_APP_REGISTRATION_ENABLED`, `SF_APP_LEGAL_DOCUMENTS_ENABLED`, and `SF_APP_ENABLE_LIMITS` accept `true` or `false`, and
`SF_APP_DEFAULT_ROLE` accepts `user`, `pro`, or `administrator`. Invalid values prevent the backend from starting. These settings are read
at startup, so restart every application replica after changing them. `SF_APP_DEFAULT_ROLE` applies only to accounts created after it is
set; changing it never reassigns existing accounts. The maximum parsed rows in one import comes from the user's plan and is unbounded when
the plan model is disabled. Limits are off unless `SF_APP_ENABLE_LIMITS=true`; enabling it activates stored plan limits, quota enforcement,
plan/usage presentation, and permits dormant-account deletion when that feature's own switch is also enabled.

The legal documents are disabled unless `SF_APP_LEGAL_DOCUMENTS_ENABLED=true`. The bundled English and Brazilian Portuguese text is
written for the public instance operated by this project's maintainer. A self-hosting operator who wants to present legal documents must
replace that content with terms and a privacy policy that identify and describe their own operation before enabling the switch.
...
All application files are stored below `SF_APP_FILE_STORAGE_PATH`. The application validates this directory at startup and refuses to start
if it cannot create, read, and write files there.

- Back up the file-storage volume together with Postgres. Losing either one can leave file references or content incomplete.
- When running more than one application replica, every replica must mount the same shared filesystem at `SF_APP_FILE_STORAGE_PATH`.
  Independent local disks are not supported.
- The development backend uses `./data/files` from `dev.env`; this directory is created automatically and should remain outside source
  control.
